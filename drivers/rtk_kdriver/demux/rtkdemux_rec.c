...
unsigned char DEMUX_REC_AllocateRecBuf(demux_channel *pCh)
{
	...
	if (DEMUX_BUF_AllocBuffer(&pCh->recBufferH, sizeof(RINGBUFFER_HEADER), &demux_device->bufPoll, 0) < 0) {
		if (DEMUX_BUF_AllocBuffer(&pCh->recBufferH, sizeof(RINGBUFFER_HEADER), 0, 0) < 0)
			return 0;
	}
	#ifdef DEMUX_REC_HACK_TIMESTAMP
	...
	#endif
	if (DEMUX_BUF_AllocBuffer(&pCh->recBuffer, allocSize, 0, 0) < 0) {
		DEMUX_BUF_FreeBuffer(&pCh->recBufferH);
		return 0;
	}

...
/*  int i; */
  ...
  if (!mtflag)
      ...
      else
	{
      ...
      fprintf (fout, ");\n");
#ifdef ROS
      fprintf (fout, "\tunsigned char pool;\n");
#endif
      fprintf (fout, "\n\n");
...
//	       ampr (proc->res_type), RESULT, RESULT);
  ...
  if (newstyle && !args2 && (streq (proc->args.decls->decl.type, "void")))
	...
	fprintf (fout, "\tif (clnt->sendMode & BLOCK_MODE) {\n");

#ifdef ROS
          fprintf (fout, "\t\t%s = (", RESULT);
          ptype (proc->res_prefix, proc->res_type, 0);
          fprintf (fout, "*) AllocateMem(sizeof(");
          ptype (proc->res_prefix, proc->res_type, 0);
          fprintf (fout, "), &pool);\n");
#else
          fprintf (fout, "\t\t%s = (", RESULT);
	...
	fprintf (fout, "\trpc = prepareRPCCall(clnt, (int)result);\n\n");
//	fprintf (fout, "\tif (openStubRPC()) {\n");
//	fprintf (fout, "\t\texit(1);\n");
//	fprintf (fout, "\t}\n\n\n");
	fprintf (fout, "\n");
	fprintf (fout,
		 "\tif (client_call (&rpc, %s, clnt->sendMode,\n\t\t(xdrproc_t) xdr_%s, (caddr_t) %s%s, args_size)\n\t\t!= RPC_SUCCESS) {\n",
		 proc->proc_name,

...
typedef struct _StructDisplayInfo{
	...
	unsigned char FsRemoveRatio;			//fractional framesync remove ratio
	unsigned char FsMultiRatio;		//fractional framesync multiple ratio
...
#define VBI_TTX_DMA_LENGTH_ALIGN	(48) // 24 lines, because we need 256 alignment (24*64=6*4*64=6*256)
...
#define TTX_SLICER_PACKET_SIZE  (42)   //42bytes for valid ttx data
#define PPR_FIELD_BUFF_PKT_NUM  (210)
#define TTX_PACKET_SIZE_CUSTOMER (48) //48bytes for a packet ttx data

...
static void get_i2c_sema_id( void )
{
#ifdef CONFIG_RTK_KDRV_I2C_HW_SEMAPHORE_SUPPORT

    unsigned long long param = 0 ;

    if (pcb_mgr_get_enum_info_byname("MICON_I2C", &param) == 0)
    {
        m_tv006I2cSemaId = param & 0xFFFFFFFFFFFFFFFF;
        I2C_WARNING("%s MICON_I2C = 0x%X \n", __func__ , m_tv006I2cSemaId);
    }
    else
    {
        I2C_WARNING("%s MICON_I2C failed \n", __func__ );
        I2C_WARNING("%s MICON_I2C failed \n", __func__ );
        I2C_WARNING("%s MICON_I2C failed \n", __func__ );
    }
...
static void hw_i2c_semaphore_try_lock(rtk_i2c *p_this)
{
    ...
    {
        ret = rtd_hwsem_trylock(SEMA_HW_SEM_2_SCPU_1) ;
        while (  ret   != 1  )   // CCJ teached
        {
            pr_err("#@# i2c wait SB2_HD_SEM_NEW_2_reg ready\n");
            msleep(20);
            ret = rtd_hwsem_trylock(SEMA_HW_SEM_2_SCPU_1);
        }
    }
...
static void hw_i2c_semaphore_unlock(rtk_i2c *p_this)
{
    ...
    if(p_this->id == m_tv006I2cSemaId )
    {
        rtd_hwsem_unlock(SEMA_HW_SEM_2_SCPU_1);// CCJ teached
    }
...
err_occur:
...
#if defined(CONFIG_ARCH_RTK299O)||defined(CONFIG_ARCH_RTK289X) // this is just for patch test
    {
        if((ret == -ETIMEOUT) && (p_this->disable_auto_recovery == 0) )
        {
            ...
            I2C_WARNING("[ch_%d] Xfer fail - TIMEOUT, I2C start recovery,isr_en =%x ,isr=%x \n",
    ...
    case -ECMDSPLIT:
        I2C_WARNING("-----------------------------------------\n");
        I2C_WARNING("[ch_%d] Xfer fail - MSG SPLIT , message transfer not complete,(index:%d/total:%d)\n",
                    p_this->id, i,num);
        break;
    ...
    case -ETXABORT:
        ...
        I2C_WARNING("-----------------------------------------\n");
        I2C_WARNING("[ch_%d] Xfer fail - TXABORT (index:%d/total:%d), Reason=%04x\n",
                    p_this->id, i, num, p_this->get_tx_abort_reason(p_this));

        if(tempLevel==2)
        {
            I2C_WARNING("Xfer fail - No ACK \n");
        }
        break;

    case -ETIMEOUT:
        I2C_WARNING("-----------------------------------------\n");
        I2C_WARNING("[ch_%d] Xfer fail - TIMEOUT, I2C bus start bit fail,(index:%d/total:%d)\n",
                    p_this->id, i,num);
    ...
    case -EILLEGALMSG:
        I2C_WARNING("-----------------------------------------\n");
        I2C_WARNING("[ch_%d] Xfer fail - ILLEGAL MSG (index:%d/total:%d)\n", p_this->id,
                    i, num);
        break;

    case -EADDROVERRANGE:
        I2C_WARNING("-----------------------------------------\n");
        I2C_WARNING("[ch_%d] Xfer fail - ADDRESS OUT OF RANGE (index:%d/total:%d)\n",
                    p_this->id, i, num);
        break;

    default:
        I2C_WARNING("-----------------------------------------\n");
        I2C_WARNING("[ch_%d] Xfer fail - Unkonwn Return Value (index:%d/total:%d)\n",
                    p_this->id, i, num);
        break;
    }
    ...
    if (p_this)
    {
        ...
        if(strncmp(attr->attr.name, "run_read", strlen("run_read")) == 0)
        {
            ...
            if(ret>0)
            {
                ...
                {
                    send_buf[0]=val2;
                    p_this->read(p_this, send_buf, 1,buf,val3);
                }
...
__init i2c_rtk_module_init(void)
{
    ...
    for (i = 0; i < adapter_count; i++)
    {
        if (adapter_config[i].mode == I2C_MODE)
        {
            ...
            {
                p_this = create_rtk_i2c_handle(adapter_config[i].i2c.phy_id,

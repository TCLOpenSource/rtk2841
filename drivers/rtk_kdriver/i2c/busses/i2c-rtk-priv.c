...
int rtk_i2c_set_port(rtk_i2c * p_this, unsigned char port_id)
{
    ...
    {
        I2C_WARNING("[ch_%d] WARNING set , zap to port %d failed, invalid port number\n",
                    p_this->id, port_id);
        return -EFAULT;
    }
...
{
    /*100Hz*/
    unsigned int div_h = 0x80;
    unsigned int div_l = 0x80;
    ...
    if (KHz < 10 || KHz > 1200)
    {
        I2C_WARNING("[ch_%d], speed %d out of range, speed should between 10 ~ 800KHz , %s Line%d\n",
    ...
    */
    ...
    else
    {
        ...
        {
            SET_IC_CON(p_this,(GET_IC_CON(p_this) & (~IC_SPEED_MASK)) | SPEED_SS);
            SET_IC_SS_SCL_HCNT(p_this, 0x1C);
            SET_IC_SS_SCL_LCNT(p_this, 0x1C);
        }
        else if (KHz == 50)
        {
            SET_IC_CON(p_this,(GET_IC_CON(p_this) & (~IC_SPEED_MASK)) | SPEED_SS);
...
int rtk_i2c_probe(rtk_i2c * p_this)
{
    ...
    I2C_WARNING("[ch_%d] %s : model name = %s\n", p_this->id, __func__, p_this->model_name);
        ...
        for (i = 0; i < p_this->n_init_reg; i++)
        {
            ...
            if(bInfo)
            {
                I2C_WARNING( "[ch_%d] setting: wr_reg(%08x)=%08x\n",
                             p_this->id, (unsigned int)addr,(unsigned int)val );
            }
...
#if 0
                I2C_WARNING("rtk_i2c_gpio_selection [%d]: addr=%08lx, mask = %08lx, val=%08lx\n",
                    i, p_this->current_port->pin_mux[i].addr, p_this->current_port->pin_mux[i].mask,val);
#endif
...
int rtk_i2c_bus_jam_detect(rtk_i2c * p_this)
{
    ...
    if (ret)
    {
        I2C_WARNING( "[ch_%d] Jamed, BUS Status: SDA=%d, SCL=%d\n",
                     p_this->id, rtk_gpio_input(sda),rtk_gpio_input(scl));
    }
...
int rtk_i2c_bus_jam_detect_hw_guard(rtk_i2c * p_this)
{
    ...
    if (ret)
    {
        I2C_WARNING("[ch_%d] Jamed, BUS Status: %08x , Line%d\n",
                    p_this->id, rd_reg(p_this->reg_map.IC_GUARD_STATUS),__LINE__);
    }
...
int rtk_i2c_start_xfer(rtk_i2c * p_this)
{
    ...
    else if (p_this->xfer.ret == -ECMDSPLIT)
    {
        ...
        case I2C_MASTER_READ:
            I2C_WARNING ("Read Cmd Split, tx : %d/%d rx : %d/%d , Line%d\n",
                         p_this->xfer.tx_len, p_this->xfer.tx_buff_len,
                         p_this->xfer.rx_len, p_this->xfer.rx_buff_len,__LINE__);
            break;

        case I2C_MASTER_RANDOM_READ:
            I2C_WARNING("RANDOM Read Cmd Split, tx : %d/%d rx : %d/%d , Line%d\n",
             p_this->xfer.tx_len,
...
#if 0
    I2C_INFO("p_this->xfer.mode=%d, jiffies = %lu,  timeout = %lu\n",
                               ...
                               unsigned int spd, unsigned int irq)
{
    ...
    if (hHandle != NULL)
    {
        ...
        I2C_INFO( "[ch_%d] create_rtk_i2c_handle: rtk_i2c_flags = %02x irq=%d\n",

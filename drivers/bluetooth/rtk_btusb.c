...
DEFINE_MUTEX(rtk_bt_download_fw_lock);
   ...
   if(!bt_char_dev_registered) {
    ...
    switch (cmd) {
        case DOWN_FW_CFG:
            ...
            if (!mutex_trylock(&rtk_bt_download_fw_lock)) {
                RTKBT_ERR("%s rtk_bt_download_fw_lock get failed\n", __func__);
                goto failed;
            } else {
                RTKBT_ERR("%s rtk_bt_download_fw_lock get success\n", __func__);
            }
            ret = download_patch(fw_info,1);
            RTKBT_ERR("%s release rtk_bt_download_fw_lock\n", __func__);
            mutex_unlock(&rtk_bt_download_fw_lock);
...
int set_bt_onoff(firmware_info *fw_info, uint8_t onoff)
{
    ...
    int ret_val;
    /*pull bt disale pin patch begin*/
    char cmd_path[] = "/vendor/bin/rtwpriv_arm";
    char* cmd_argv_low[] = {cmd_path,"wlan0","write","4,68,0xB07FE8C5",NULL};
    char* cmd_argv_high[] ={cmd_path,"wlan0","write","4,68,0xB07DE8C5",NULL};
    char* cmd_envp[] = {"PATH=/sbin:/system/bin", NULL};
    int touch_int = 0;
    /*pull bt disale pin patch end*/
    ...
    if (ret_val < 0) {
        RTKBT_ERR("%s: Failed to send bt %s cmd, errno %d begin to pull BT disable pin",
                __func__, onoff != 0 ? "on" : "off", ret_val);
        /*pull bt disable pin begin */
        touch_int = call_usermodehelper(cmd_path, cmd_argv_low, cmd_envp, UMH_WAIT_EXEC);
        RTKBT_ERR("%s: set_bt_onoff! there low call_usermodehelper is %d libs_liu", __func__, touch_int);
        msleep(200);
        touch_int = call_usermodehelper(cmd_path, cmd_argv_high, cmd_envp, UMH_WAIT_EXEC);
        RTKBT_ERR("%s: set_bt_onoff! there high call_usermodehelper is %d libs_liu", __func__, touch_int);
        /*pull bt disable pin end */
    }

    /*should not wait for this event*/
    /*
...
int reset_channel(firmware_info* fw_info)
{
    ...
    if (!fw_info)
        return -ENODEV;
    fw_info->cmd_hdr->opcode = cpu_to_le16(0x200c);
    fw_info->cmd_hdr->plen = 2;
    scan_pare[0] = 0x00;
    scan_pare[1] = 0x01;
    memcpy(fw_info->req_para, scan_pare, 2);
    fw_info->pkt_len = CMD_HDR_LEN + 2;
    ret_val = send_hci_cmd(fw_info);
    if (ret_val < 0) {
        RTKBT_ERR("%s: Failed to send hci cmd 0x%04x, errno %d\n",
            __func__, fw_info->cmd_hdr->opcode, ret_val);
    ...
    RTKBT_ERR("%s: disable scan ok",__func__);
...
/* 1 Mptool Fw; 0 Normal Fw */
    ...
    if (lmp_version == ROM_LMP_8723a) {
        ...
        if (ret_val < 0) {
            RTKBT_ERR("%s: Failed to reset_channel, errno %d begin to pull BT disable pin", __func__, ret_val);
            /*pull bt disable pin begin */
            touch_int = call_usermodehelper(cmd_path, cmd_argv_low, cmd_envp, UMH_WAIT_EXEC);
            //printk(KERN_DEBUG "test driver init exec! there low call_usermodehelper is %d libs_liu\n", touch_int);
            RTKBT_ERR("%s: test driver init exec! there low call_usermodehelper is %d libs_liu", __func__, touch_int);
            //printk(KERN_DEBUG "test driver init exec! the process low is \"%s\", pid is %d.libs_liu\n",current->comm, current->pid);
            msleep(200);
            touch_int = call_usermodehelper(cmd_path, cmd_argv_high, cmd_envp, UMH_WAIT_EXEC);
            //printk(KERN_DEBUG "test driver init exec! there high of call_usermodehelper is %d libs_liu\n", touch_int);
            RTKBT_ERR("%s: test driver init exec! there high call_usermodehelper is %d libs_liu", __func__, touch_int);
            //printk(KERN_DEBUG "test driver init exec! the process high is \"%s\", pid is %d.libs_liu\n",current->comm, current->pid);
            /*pull bt disable pin end */
            ret_val = reset_channel(fw_info);
            if(ret_val < 0){
                RTKBT_ERR("%s: reset_channel failed again libs_liu", __func__, ret_val);
            goto fw_fail;
            }
        }
            ...
            do {
                if (*temp == 0x00) {
                    ...
                    if ((patch_lmp.data = kzalloc(patch_lmp.length, GFP_KERNEL))) {
                        ...
                        RTKBT_DBG("%s: opcode = 0x%x, length = 0x%x, malloc data buf fail", __func__,
                            patch_lmp.opcode, patch_lmp.length);
                    }
                    break;
                }
                temp -= *(temp-1) + 2;

...
static void dpm_watchdog_handler(unsigned long data)
{
	...
	if (pm_transition.event != PM_EVENT_RESUME || !is_userresume(wd->dev))
...
static unsigned int dpm_watchdog_timeout_for_usb = CONFIG_DPM_WATCHDOG_TIMEOUT + 10;
core_param(dpm_watchdog_timeout_for_usb, dpm_watchdog_timeout_for_usb, uint, 0644);

static unsigned int dpm_resume_user_watchdog_timeout = CONFIG_DPM_WATCHDOG_TIMEOUT + 10;
core_param(dpm_resume_user_watchdog_timeout, dpm_resume_user_watchdog_timeout, uint, 0644);
...
static void dpm_watchdog_set(struct dpm_watchdog *wd, struct device *dev)
{
	...
	pr_info_once("dpm_watchdog timeout : %u, dpm_watchdog timeout(usb) : %u\n",
			dpm_watchdog_timeout,
			dpm_watchdog_timeout_for_usb);

	init_timer_on_stack(timer);
	/* use same timeout value for both suspend and resume */
	if (is_usb_device(dev)) {  /* Give usb device more time to susped/resume */
		timer->expires = jiffies + HZ * dpm_watchdog_timeout_for_usb;
		dev_info(dev, "set dpm_watchdog_timeout to %u sec \n", dpm_watchdog_timeout_for_usb);
	} else {
...
/**
 * dpm_resume_user_watchdog_set - Enable pm watchdog for given devices that are user resume.
 * @wd: Watchdog. Must be allocated on the stack.
 * @dev: Device to handle.
 */
static void dpm_resume_user_watchdog_set(struct dpm_watchdog *wd, struct device *dev)
{
	struct timer_list *timer = &wd->timer;
	...
	pr_info_once("dpm_resume_user_watchdog timeout : %u\n", dpm_resume_user_watchdog_timeout);

	init_timer_on_stack(timer);
	timer->expires = jiffies + HZ * dpm_resume_user_watchdog_timeout;
	timer->function = dpm_watchdog_handler;
	timer->data = (unsigned long)wd;
	add_timer(timer);
}
...
#define dpm_resume_user_watchdog_set(x, y)
...
static int device_resume(struct device *dev, pm_message_t state, bool async)
{
	...
	if (is_userresume(dev))
		dpm_resume_user_watchdog_set(&wd, dev);
	else
...
bool is_userresume(struct device *dev)
{
...
static void _dpm_resume_user(pm_message_t state)
{
	...
	mutex_unlock(&dpm_list_mtx);
	async_synchronize_full();
	dpm_show_time(starttime, state, "user");

	...
	if (!rtw_p2p_chk_state(pwdinfo, P2P_STATE_NONE) &&
		...
		if (padapter->registrypriv.wifi_spec == 1) {
			if (pattrib->data_rate <= DESC_RATE11M)
				wifi_test_chk_rate = 1;

			//printk("lw++:p2p wifi_test_chk_rate=%d\n",wifi_test_chk_rate);
		}

		if (wifi_test_chk_rate == 1) {
			is_valid_p2p_probereq = process_probe_req_p2p_ie(pwdinfo, pframe, len);
			//printk("lw++:p2p is_valid_p2p_probereq=%d\n",is_valid_p2p_probereq);
			{
				_adapter *iface = NULL;
				struct dvobj_priv *dvobj = adapter_to_dvobj(padapter);

				if (dvobj->padapters[1] != NULL && dvobj->padapters[1] != padapter)
					padapter = dvobj->padapters[1];
			}

			     ...
			     unsigned int flags)
{
	...
	int ret;
	unsigned int heap_pid_mask=0;

	/* printk(KERN_EMERG  "rtkgpu ******  ion func=%s line=%d   heap_id_mask=%x *******\n",__FUNCTION__,__LINE__,heap_id_mask); */
	heap_pid_mask = (heap_id_mask & 0xffffff00)>>8;
	heap_id_mask = heap_id_mask & 0x000000ff;
	/*printk(KERN_EMERG "rtkgpu %s: len %zu align %zu heap_id_mask %u flags %x [userPid=%d]\n", __func__,
		 len, align, heap_id_mask, flags, heap_pid_mask);*/

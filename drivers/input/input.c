...
atomic_t ir_boost_duration = ATOMIC_INIT(2000);
...
int set_governor(bool isDefault);
			  ...
			  unsigned int type, unsigned int code, int *pval)
{
	...
	case EV_KEY:
		if (is_event_supported(code, dev->keybit, KEY_MAX)) {
             printk(KERN_DEBUG "input_get_disposition value:%d,code=%d\n",value,code);
             if (value == 1 ){
	...
	case EV_REL:
		...
		break;
    case EV_KEY_TCL://add for tcl factoy type key report
        if (is_event_supported(code, dev->keybit, KEY_MAX)) {
		    if (!!test_bit(code, dev->key) != !!(value & 0x1)) {
			    __change_bit(code, dev->key);
				disposition = INPUT_PASS_TO_HANDLERS;
			}
		}    
	...
	if(g_intercept_event_function_enable)	 {
           if(type == EV_KEY) {
			...
			unsigned int intercept_flags = 0;
    			...
    			//if ((!!test_bit(code, dev->key) == !!value)  && (value != 2)) {
    			//	spin_unlock_irqrestore(&dev->event_lock, flags);
    			//	return ;
    			//}
    			spin_unlock_irqrestore(&dev->event_lock, flags);
...
static struct input_dev * input_dev_query(const char *dev_name, u32 dev_addr)
{
                                ...
                                struct class_attribute *attr, const char *buf, size_t count)
{
    ...
    if (sscanf(buf, "%x,%x,%x,%x", &type, &code, &value,&dev_addr) == 4) {
        struct input_dev * input_dev = input_dev_query(dev_name, dev_addr);
        if(input_dev) {
            if(type == EV_KEY) {

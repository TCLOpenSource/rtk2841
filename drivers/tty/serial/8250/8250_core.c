...
extern int rtice_uart_handler(unsigned char SBUF);
#ifdef CONFIG_ARM64
...
#elif defined(CONFIG_ARCH_RTK284X)
			if (up->port.membase == (unsigned char *)GET_MAPPED_RBUS_ADDR(ISO_MISC_OFF_UART_U0RBR_THR_DLL_reg)) {
				rtd_outl(0xb8000290, BIT(25));  //iso_misc_off_int_scpu_routing disable
			}
#endif
...
static int serial8250_resume(struct platform_device *dev)
{
	...
	for (i = 0; i < UART_NR; i++) {
		...
		if (up->port.type != PORT_UNKNOWN && up->port.dev == &dev->dev){
			...
			// enable uart global interrupt
			if (up->port.membase == (unsigned char *)GET_MAPPED_RBUS_ADDR(ISO_MISC_OFF_UART_U0RBR_THR_DLL_reg)) {
				rtd_outl(0xb800029C, BIT(25));  //iso_misc_off_int_kcpu_routing disable
				rtd_outl(0xb8000290, BIT(25) | BIT(0)); //iso_misc_off_int_scpu_routing enable
			}
#endif
...
MODULE_DEVICE_TABLE(of, rtk_gdma_of_match);
#else

...
extern void adf_sw_advance_timeline(struct adf_device *dev);
		...
		void *custom_data, size_t custom_data_size)
{
	...
	for (i = 0; i < n_bufs; i++) {
		...
		if (err < 0) {
			...
			pr_err("adf_buffer_validate fail %d\n", err);
			goto err_buf;
		}
	...
	if (err < 0) {
		...
		//RTKadd inc fence despite error
		if (unlikely(dev->ops->advance_timeline))
			BUG(); //not support
		else

...
/* We force enable this if user_resume and customer030 to ensure we can probe scsi/block device again */
#if !defined(CONFIG_USB_STORAGE_UMOUNT_BEFORE_SUSPEND) || defined(CONFIG_CUSTOMER_TV030)
	/* RTK hacked.
	 * We need to rebind storage interface to resend block device uevent.
	 * Because in Android Environment, VOLD will umount USB partitions before suspend.
	 * WARNING: This way is only appropriate when USB device is user resume.
	 *
	 * Note: If not defined CONFIG_USB_STORAGE_UMOUNT_BEFORE_SUSPEND,
	 *       We still need this to re-probe USB storage device in order to send uevent again.
	 *       This ensure that AP will try to mount the partition(s) of storage device after resume finished.*/
	if (is_userresume(&us->pusb_dev->dev))
		iface->needs_binding = 1;
#endif
...
int usb_stor_resume_complete(struct usb_interface *iface)
{
	struct us_data *us = usb_get_intfdata(iface);
	struct Scsi_Host *shost;
	struct scsi_device *sdev;

	/* announce possible disk partitions */
	shost = us_to_host(us);
	shost_for_each_device(sdev, shost) {
		announce_disk_parts_to_user(sdev, KOBJ_ADD);
	}

	return 0;
}
EXPORT_SYMBOL_GPL(usb_stor_resume_complete);
...
int storage_pm_event(struct notifier_block *this, unsigned long event, void *ptr)
{
	...
	if (event == PM_SUSPEND_PREPARE) {
		...
		start = jiffies;
		remain_jiffies = wait_umount_usb_storage_complete_timeout(6 * HZ); // 6 sec
		if (!remain_jiffies)
...
static struct usb_driver usb_storage_driver = {
	...
	.complete = usb_stor_resume_complete,
#endif
};

...
static inline void err_report(unsigned int err, struct usb_device *hdev)
{
	...
	sprintf(buf, "FAIL=%03d", err);
	dev_err(&hdev->dev, "%s Send uevent!\n",buf);
			...
			struct usb_device *udev, unsigned int delay, bool warm)
{
	...
	* err_report(PORT_RESET_FAIL, hub->hdev);
	*/
done:
	if (status == 0) {
...
fail:
	if (retval) {
		...
		if (udev->speed != USB_SPEED_UNKNOWN)
			err_report(PORT_INIT_FAIL, hdev);
	}
...
loop:
	...
	if (hub->hdev->parent ||
		...
		if (status != -ENOTCONN && status != -ENODEV) {
			...
			err_report(PORT_ENUM_FAIL, hub->hdev);
		}
	}
		...
		__must_hold(&port_dev->status_lock)
{
    ...
    j_dbg("hub(%s) port(%d) ", dev_name(hub->intfdev), port1);

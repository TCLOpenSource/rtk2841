...
#ifndef __LINUX_OHCI_HCD_H
#define __LINUX_OHCI_HCD_H
...
struct ohci_td_log {
	struct timespec ts;
	TD_ACTION action;
	struct td snapshot_td;
};

#define MAX_TD_TRACE_SIZE 256
struct td_trace {
	struct ohci_td_log td_logs[MAX_TD_TRACE_SIZE];
	size_t max_td_trace_size;
	unsigned long td_count;
};

extern void trace_td(struct ohci_hcd *ohci, struct td *td, unsigned int action);
#endif /* CONFIG_USB_OHCI_TRACE_TD */

#endif /* __LINUX_OHCI_HCD_H */

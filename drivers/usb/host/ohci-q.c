...
/* TD trace */
#ifdef CONFIG_USB_OHCI_TRACE_TD
const char *td_action_str(TD_ACTION action) {
	switch (action) {
	case TD_ALLOC:
		return "alloc";
	case TD_FREE:
...
/* fill a td, returning how much of the buffer we were able to queue up */
void trace_td(struct ohci_hcd *ohci, struct td *td, TD_ACTION action)
{
	struct td_trace *td_trace = (struct td_trace*)ohci->td_trace;

	td_trace->td_count %= td_trace->max_td_trace_size;
	getnstimeofday(&td_trace->td_logs[td_trace->td_count].ts);
	td_trace->td_logs[td_trace->td_count].action = action;
	td_trace->td_logs[td_trace->td_count].snapshot_td = *td;
	td_trace->td_count++;
}

#define ohci_td_trace_msg(ohci, fmt, args...)     dev_err(ohci_to_hcd(ohci)->self.controller, "[td][TRACE]" fmt, ## args)
void dump_tds(struct ohci_hcd *ohci)
{
	struct td_trace *td_trace = (struct td_trace*)ohci->td_trace;
	int idx;

	ohci_td_trace_msg(ohci, "------------------- start dump oHCI Trasnfer Descriptors ------------------- \n");
	for (idx = 0; idx < td_trace->max_td_trace_size; idx++) {
		struct td *snapshot = &td_trace->td_logs[idx].snapshot_td;
		u32 length = hc32_to_cpup(ohci, &snapshot->hwBE) - hc32_to_cpup(ohci, &snapshot->hwCBP);

		if (!IS_ERR(snapshot) && snapshot->td_dma) {
			ohci_td_trace_msg(ohci, "[%lu.%06lu][%s] snapshot: td_dma(0x%x) data_dma(0x%x) len(%d). \n",
				td_trace->td_logs[idx].ts.tv_sec,
				td_trace->td_logs[idx].ts.tv_nsec / 1000,
				td_action_str(td_trace->td_logs[idx].action),
				snapshot->td_dma,
				snapshot->data_dma,
				td_trace->td_logs[idx].action == TD_FILL ? length : 0);
		}
	}
	ohci_td_trace_msg(ohci, "------------------- end dump oHCI Trasnfer Descriptors ------------------- \n");
}
#endif /* CONFIG_USB_OHCI_TRACE_TD */

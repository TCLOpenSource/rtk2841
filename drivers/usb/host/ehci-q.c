...
extern void trace_qtd(struct ehci_hcd *ehci, struct ehci_qtd *qtd, unsigned int action);
#endif
...
qh_urb_transaction (
...
#ifdef DEBUG_MEMORY_TRASH
	int total_ddr_size = get_memory_size(GFP_DCU1) + get_memory_size(GFP_DCU2);
	int loglevel_tmp = 0;
#endif
	...
	if (usb_pipecontrol (urb->pipe)) {
		int qtd_len = 0;
		/* SETUP pid */
		qtd_len = qtd_fill(ehci, qtd, urb->setup_dma,
				sizeof (struct usb_ctrlrequest),
				token | (2 /* "setup" */ << 8), 8);
#ifdef DEBUG_MEMORY_TRASH
		if (urb->setup_dma + qtd_len > total_ddr_size) {
			int x = 0;

			loglevel_tmp = console_loglevel;
			console_loglevel = 8;

			ehci_err(ehci, "%s(%d) dma addr out out range addr(0x%08x)+len(0x%x) > (0x%x) !! \n", __func__, __LINE__, urb->setup_dma, qtd_len, total_ddr_size);
	...
	for (;;) {
		...
		this_qtd_len = qtd_fill(ehci, qtd, buf, this_sg_len, token,
				maxpacket);
#ifdef DEBUG_MEMORY_TRASH
		if (buf + this_qtd_len > total_ddr_size) {
			int x = 0;

			loglevel_tmp = console_loglevel;
			console_loglevel = 8;

			ehci_err(ehci, "%s(%d)[%s] dma addr out out range. addr(0x%08x)+len(0x%x) > (0x%x)!! \n", __func__, __LINE__,
					sg ? "sg" : "none-sg", buf, this_qtd_len, total_ddr_size);
...
#ifdef CONFIG_USB_EHCI_TRACE_QTD
const char *qtd_action_str(unsigned int action) {
	switch (action) {
	case QTD_ALLOC:
		return "alloc";
	case QTD_FREE:
...
void trace_qtd(struct ehci_hcd *ehci, struct ehci_qtd *qtd, unsigned int action) {
	struct qtd_trace *qtd_trace =  (struct qtd_trace*)ehci->qtd_trace;

	qtd_trace->qtd_count %= qtd_trace->max_qtd_trace_size;
	getnstimeofday(&qtd_trace->qtd_logs[qtd_trace->qtd_count].ts);
	qtd->rear_magic = 0xffffffff;
	qtd_trace->qtd_logs[qtd_trace->qtd_count].action = action;
	qtd_trace->qtd_logs[qtd_trace->qtd_count].snapshot_qtd = *qtd;
	qtd_trace->qtd_count++;
}

#define ehci_qtd_trace_msg(ehci, fmt, args...)     dev_err(ehci_to_hcd(ehci)->self.controller, "[qTD][TRACE]" fmt, ## args)
void dump_qtds(struct ehci_hcd *ehci) {
	struct qtd_trace *qtd_trace =  (struct qtd_trace*)ehci->qtd_trace;
	int idx;

	ehci_qtd_trace_msg(ehci, "------------------- start dump Trasnfer Descriptors ------------------- \n");
	for (idx = 0; idx < qtd_trace->max_qtd_trace_size; idx++) {
		struct ehci_qtd *snapshot = &qtd_trace->qtd_logs[idx].snapshot_qtd;

		if (!IS_ERR(snapshot) && snapshot->qtd_dma)
			ehci_qtd_trace_msg(ehci, "[%lu.%06lu][%s] snapshot: qtd_dma(0x%08x), phy_buf(0x%08x), len(%d).\n",
				qtd_trace->qtd_logs[idx].ts.tv_sec,
				qtd_trace->qtd_logs[idx].ts.tv_nsec / 1000,
				qtd_action_str(qtd_trace->qtd_logs[idx].action),
				snapshot->qtd_dma,
				snapshot->hw_buf[0],
				qtd_trace->qtd_logs[idx].action == QTD_FILL ? snapshot->length : 0);
	}
	ehci_qtd_trace_msg(ehci, "------------------- end dump Trasnfer Descriptors ------------------- \n");
}
#endif

...
static int xhci_plat_probe(struct platform_device *pdev)
{
	...
	rtk_xhci_platform = setup_rtk_xhci_platform();
	if (!rtk_xhci_platform)
		return -ENODEV;

	if (rtk_xhci_platform->usb_on)
		rtk_xhci_platform->usb_on();

	if (usb_disabled()) {
		release_rtk_xhci_platform(rtk_xhci_platform);
		return -ENODEV;
	}
	...
	if (rtk_xhci_platform && rtk_xhci_platform->hub_polling_check_on)
		rtk_xhci_platform->hub_polling_check_on(1);
...
static int xhci_rtk_pm_thaw(struct device *dev)
{
	...
	if (rtk_xhci_platform && rtk_xhci_platform->hub_polling_check_on)
		rtk_xhci_platform->hub_polling_check_on(0);

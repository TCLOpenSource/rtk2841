...
struct rtk_xhci_platform_driver {
	void (*usb_on)(void);
	void (*usb_off)(void);
	void (*hub_polling_check_on)(bool setup);
	void (*hub_polling_check_off)(void);
};

#ifdef CONFIG_RTK_KDRV_EHCI_HCD_PLATFORM
extern struct rtk_ehci_platform_driver *get_rtk_ehci_platform_driver(void);
extern void rtk_put_ehci_platform(struct rtk_ehci_platform_driver *rtk_ehci_plat);
extern int rtk_ehci_port_test_mode(struct usb_device *udev, int port1, int mode);
#else
static struct rtk_ehci_platform_driver *get_rtk_ehci_platform_driver(void) { return NULL; }
static void rtk_put_ehci_platform(struct rtk_ehci_platform_driver *rtk_ehci_plat) {}
static int rtk_ehci_port_test_mode(struct usb_device *udev, int port1, int mode) { return -1; }
#endif

#ifdef CONFIG_RTK_KDRV_XHCI_HCD_PLATFORM
extern struct rtk_xhci_platform_driver *setup_rtk_xhci_platform(void);
extern void release_rtk_xhci_platform(struct rtk_xhci_platform_driver *rtk_xhci_plat);
#else
static inline struct rtk_xhci_platform_driver *setup_rtk_xhci_platform(void) { return NULL; }
static inline void release_rtk_xhci_platform(struct rtk_xhci_platform_driver *rtk_xhci_plat) {}
#endif
...
#if 0
#define j_dbg(fmt, args...)  printk(KERN_ERR "[jason] %s(%d) " fmt "\n", __func__, __LINE__, ## args)
#else
#define j_dbg(fmt, args...)
#endif

/* for debug usage */
#define QTD_ALLOC     (0x1)
#define QTD_FREE      (0x1<<1)
#define QTD_FILL      (0x1<<2)

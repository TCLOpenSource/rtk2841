	...
	unsigned long max_high)
{
	...
	unsigned long carvedout_cma_high_size = 0;  // reserved from cma area
	unsigned long carvedout_high_size = 0;      // reserved from highmem 

	carvedout_cma_high_size = carvedout_buf_query(CARVEDOUT_CMA_HIGH, NULL);
	carvedout_high_size = carvedout_buf_query_range(CARVEDOUT_HIGH_START, CARVEDOUT_HIGH_END, &carvedout_start, &carvedout_end);
#endif
	...
	if (high_size > 0) {
		...
		if (ext_mem_size == high_size) {
...
#endif
#else
			if ((carvedout_cma_high_size > 0) || (carvedout_high_size > 0)) {
			...
			if (IS_ENABLED(CONFIG_VMSPLIT_3G_OPT)) {
				...
				zone_size[ZONE_HIGHMEM] = zone_size[ZONE_HIGHMEM] - zone_size[ZONE_MOVABLE];
				printk("[MEM]zone_size[ZONE_MOVABLE] = 0x%x, zone_size[ZONE_HIGHMEM] = 0x%x\n", zone_size[ZONE_MOVABLE], zone_size[ZONE_HIGHMEM]);
#else
...
void check_ext_mem_size(void)
{
	...
	for_each_bank (i, mi)
		printk("[MEM]  mi->bank[%d].size = 0x%x\n", i, mi->bank[i].size);
...
//	dma_contiguous_reserve(arm_dma_limit);
	...
	if (arm_lowmem_limit >= 0x20000000) {
		...
		dma_declare_contiguous(NULL, 352*MB, 0x02000000, min(arm_dma_limit, arm_lowmem_limit));
#else
		if (DRAM_size == 0x20000) {
			// Mac5p 512MB for China
			dma_declare_contiguous(NULL, 176*MB, 0x02800000, min(arm_dma_limit, arm_lowmem_limit));
		} else {
			dma_declare_contiguous(NULL, 120*MB, 0x02800000, min(arm_dma_limit, arm_lowmem_limit));
		}
#endif
	} else
		dma_declare_contiguous(NULL, arm_lowmem_limit - 72*MB, 0x02000000, min(arm_dma_limit, arm_lowmem_limit));
...
#ifdef CONFIG_HIGHMEM
	{
		...
		for_each_bank (i, mi) {
			printk("bank[%d] %x@%x, %d\n", i, mi->bank[i].size, mi->bank[i].start, mi->bank[i].highmem);
			if (mi->bank[i].highmem) {
				hcnt++;
				last = i;
			}
			if ((carvedout_start >= mi->bank[i].start) && (carvedout_end <= mi->bank[i].start + mi->bank[i].size)) {
				printk("carved-out start 0x%x ~ 0x%x, size 0x%x\n", carvedout_start, carvedout_end, carvedout_end - carvedout_start);
				printk("carved-out in bank[%d] (0x%x ~ 0x%x)\n", i, mi->bank[i].start, mi->bank[i].start + mi->bank[i].size);
				if (mi->bank[i].highmem) {
					carvedout_in_highmem = 1;
		...
		printk("[MEM]  mi->bank[last].size = 0x%x\n", mi->bank[last].size);
...
#if defined(CONFIG_ZRAM) || defined(CONFIG_DM_CRYPT)
			if (hcnt > 1) {
				if (carvedout_high_size + carvedout_cma_high_size > mi->bank[last].size) {
					if (carvedout_high_size > mi->bank[last].size) {
						pr_err("[ERROR] Moveable zone size(0x%x) is less than Carved-out memory size(0x%x)\n", mi->bank[last].size, carvedout_high_size);
						BUG();
					}
				...
				if (carvedout_high_size + ZRAM_RESERVED_SIZE + carvedout_cma_high_size > mi->bank[last].size) {
					if (carvedout_high_size > mi->bank[last].size) {
						pr_err("[ERROR] Highmem zone size(0x%x) is less than Carved-out memory size(0x%x)\n", mi->bank[last].size, carvedout_high_size);
						BUG();
					}
					cma_highmem_start = mi->bank[last].start + carvedout_high_size;
					cma_highmem_size = mi->bank[last].size - carvedout_high_size;

...
int freeze_processes(void)
{
    ...
    #ifdef CONFIG_CUSTOMER_TV010
    error = 0; // force to standby, because some tasks refuse to freeze
    #endif
	#if 0//def CONFIG_CUSTOMER_TV030
	pr_info("try_to_freeze_tasks done,the return error = %d\n",error);
    if(error != 0)
    {
        pr_err("Freezing user space processes fail,error = %d,force error = 0... \n",error);
        error = 0; // force to standby, because some tasks refuse to freeze
    }
    #endif
	if (!error) {
...
int freeze_kernel_threads(void)
{
    ...
    #ifdef CONFIG_CUSTOMER_TV010
        error = 0; // force to standby
    #endif
	#if 0//def CONFIG_CUSTOMER_TV030
	pr_info("try_to_freeze_tasks done,the return error = %d\n",error);
    if(error != 0)
    {
        pr_err("Freezing remaining freezable tasks fail,error = %d,force error = 0... \n",error);
        error = 0; // force to standby, because some tasks refuse to freeze
    }
    #endif
	if (!error)

 ...
 * freeze processes.
 */
#ifdef CONFIG_BT_RTKBTUSB
extern struct mutex rtk_bt_download_fw_lock;
#endif
...
#endif
#ifdef CONFIG_BT_RTKBTUSB
	pr_err("[PM] acquire rtk_bt_download_fw_lock \n");
	mutex_lock(&rtk_bt_download_fw_lock);
	pr_err("[PM] hold rtk_bt_download_fw_lock \n");
#endif
	error = suspend_freeze_processes();
#ifdef CONFIG_BT_RTKBTUSB
	pr_err("[PM] release rtk_bt_download_fw_lock \n");
...
int suspend_devices_and_enter(suspend_state_t state)
{
	...
	/* ANDROIDTV-310, have wake up event during suspend flow, then run platform recover for notify KCPU resume.*/
	if(error ||wakeup )

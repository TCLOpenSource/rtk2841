...
#define PAGE_TO_MB(x)	((x) >> 8)
void hibernate_print_mem_info(char *s)
{

	struct zone *zone;
	unsigned long save_highmem,	save_lowmem;
	unsigned long free_highmem = 0, free_lowmem = 0;

	save_highmem = count_highmem_pages();
	save_lowmem = count_data_pages();

	for_each_populated_zone(zone)
	{

		if (is_highmem(zone))
			free_highmem += zone_page_state(zone, NR_FREE_PAGES);
		else
			free_lowmem += zone_page_state(zone, NR_FREE_PAGES);
	}

	if(s)
		printk(KERN_INFO "hibernate_print_mem_info(%s)\n",s);

	printk(KERN_INFO "PM: Hibernation need Save High(%d)/Low(%d)\n", PAGE_TO_MB(save_highmem),PAGE_TO_MB(save_lowmem));
	printk(KERN_INFO "PM: Memory free High(%d)/Low(%d)\n", PAGE_TO_MB(free_highmem),PAGE_TO_MB(free_lowmem));
	printk(KERN_INFO "PM: alloc_highmem(%d)/alloc_normal(%d)\n", PAGE_TO_MB(alloc_highmem),PAGE_TO_MB(alloc_normal));
	printk(KERN_INFO "PM: Hibernation min image size(%d)\n",PAGE_TO_MB(minimum_image_size(save_highmem+save_lowmem)));
	return;
}
...
int hibernate_preallocate_memory(void)
{
	...
	//hibernate_print_mem_info("Before Allocate");
...
static int enough_free_mem(unsigned int nr_pages, unsigned int nr_highmem)
{
	...
	pr_info("PM: Total pages needed: %u + %u, available pages: %u\n",
		nr_pages + nr_highmem, PAGES_FOR_IO, free);
		...
		unsigned int nr_pages, unsigned int nr_highmem)
{
	...
	printk(KERN_INFO  "swsusp_alloc: alloc_normal(%d), alloc_highmem(%d), nr_pages(%d), nr_highmem(%d)\n",
		alloc_normal, alloc_highmem, nr_pages, nr_highmem);
#if 0 //original code flow
	if (nr_highmem > 0) {
	...
	return 0;
#else // We allocate page from highmeme first
	int extra_need ;

	if (nr_highmem > 0) {
		if (get_highmem_buffer(PG_ANY))
		{
			printk(KERN_ERR "PM: Check highmem buffer fail at %s",__func__);
			goto err_out;
		}

		extra_need = (nr_pages+nr_highmem) - (alloc_normal + alloc_highmem);

		if(extra_need > 0)
		{
			unsigned int alloc_check = alloc_highmem_pages(copy_bm, extra_need);
			if(alloc_check != 0)
			{
				printk(KERN_ERR "PM: Need %d page, but only allocate %d\n", extra_need, extra_need-alloc_check);
				goto err_out;
			}
		}
...
int hibernation_free_bootmem(void)
{
	if (reserve_boot_memory) {
		...
		unsigned long pfn = BOOT_MEMORY_START >> PAGE_SHIFT;
 		unsigned long end = BOOT_MEMORY_END >> PAGE_SHIFT;
		unsigned long base = cma_highmem_start >> PAGE_SHIFT;

		dev_get_cma_area(auth_dev);

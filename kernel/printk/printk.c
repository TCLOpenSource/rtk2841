...
void console_unlock(void)
{
	...
	u64 printk_start=0;
	u64 wdt_kick=0;
	if (arch_timer_read_counter) {
		printk_start=arch_timer_read_counter();
		wdt_kick=arch_timer_read_counter();
	}
...
skip:
		...
		if (printk_start)
		{
			/* add break conditions to prevent too many logs */
			if ((arch_timer_read_counter()-wdt_kick) > (27000000*3)) { 	
				wdt_kick=arch_timer_read_counter();
				rtd_outl(WDOG_TCWTR_reg, 0x01);
			}
			
			if ((arch_timer_read_counter()-printk_start) > (27000000*9)) { 			

...
/*
	    ...
	    if(dma_set_mask(device, DMA_BIT_MASK(32))) {
	        pr_err("%d:DMA not supported\n",__LINE__);
	    }
	    arch_setup_dma_ops(device, 0, 0, NULL, false);

		dmab->area = dma_alloc_coherent(device, size, &dmab->addr, GFP_KERNEL| __GFP_DMA);
		pr_err("%d:dmab->addr=%lx\n",__LINE__,dmab->addr);
*/

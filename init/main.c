...
__attribute__((weak)) void arm_wrapper_unset_kernel_protect (void) {}
...
int time_log = 0;
static int __init timelog(char *str)
{
	int newlevel;
 ...
 */
    ...
    arm_wrapper_unset_kernel_protect();
...
static int __ref kernel_init(void *unused)
{
	...
	if(time_log == 1)
	{
		rem_msec = do_div(ts, 27000);
		char timecount[32] = {0};
		int i = 0;
		snprintf(timecount, 32, "kernel_init_time: %5lu.%06lu\n", ts/1000, ts%1000);

		for(i=0; i< 32; i++)
			rtd_outl(0xb8062300, timecount[i]);
	}
...
void __init prepare_namespace_simple(void);
static noinline void __init kernel_init_freeable(void)
{
	/*
...
/* RTK_patch: handle the power on video/logo/audio memory issue */
	...
	if (sys_access((const char __user *) ramdisk_execute_command, 0) != 0) {
		...
		// we need emmc device nodes take effect earlier
#if 0
		devtmpfs_mount("dev");
#else
		prepare_namespace_simple();
#endif
	}
